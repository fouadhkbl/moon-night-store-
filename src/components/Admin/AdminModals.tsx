    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/95 backdrop-blur-md animate-fade-in">
      <div className="bg-[#1e232e] w-full max-w-sm rounded-[2rem] border border-gray-800 shadow-3xl p-8 animate-slide-up">
        <div className="text-center mb-8">
           <div className="w-16 h-16 bg-blue-500/10 rounded-2xl flex items-center justify-center mx-auto mb-4 text-blue-500 border border-blue-500/20"><DollarSign className="w-8 h-8" /></div>
...
export const ProductFormModal = ({ product, onClose, onSave }: { product: Partial<Product> | null, onClose: () => void, onSave: (p: any) => void }) => {
    const [formData, setFormData] = useState<Partial<Product>>(product || { name: '', description: '', price: 0, category: GameCategory.ACCOUNTS, image_url: '', image_url_2: '', stock: 1, platform: 'All Platforms', country: 'Global', is_trending: false, is_vip: false, is_hidden: false });
    const [isSaving, setIsSaving] = useState(false);
...
export const CouponFormModal = ({ coupon, onClose, onSave }: { coupon: Partial<Coupon> | null, onClose: () => void, onSave: (c: any) => void }) => {
    const [formData, setFormData] = useState<Partial<Coupon>>(coupon || { code: '', discount_type: 'percent', discount_value: 0, max_uses: null, usage_count: 0, expires_at: null, is_active: true });
    const [isSaving, setIsSaving] = useState(false);
    const handleSubmit = async (e: React.FormEvent) => { e.preventDefault(); setIsSaving(true); await onSave(formData); setIsSaving(false); };
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-fade-in">
...
export const PointProductFormModal = ({ product, onClose, onSave }: { product: Partial<PointProduct> | null, onClose: () => void, onSave: (p: any) => void }) => {
    const [formData, setFormData] = useState<Partial<PointProduct>>(product || { name: '', description: '', image_url: '', cost: 0, duration: '', advantage: '', is_active: true });
    const [isSaving, setIsSaving] = useState(false);
    const handleSubmit = async (e: React.FormEvent) => { e.preventDefault(); setIsSaving(true); await onSave(formData); setIsSaving(false); };
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-fade-in">
...
export const TournamentFormModal = ({ tournament, onClose, onSave }: { tournament: Partial<Tournament> | null, onClose: () => void, onSave: (t: any) => void }) => {
    const [formData, setFormData] = useState<Partial<Tournament>>(tournament || { title: '', game_name: '', description: '', image_url: '', start_date: new Date().toISOString(), status: 'open', entry_fee: 'Free', prize_pool: '0 DH', max_participants: 16, current_participants: 0, format: 'Single Elimination', rules: '' });
    const [isSaving, setIsSaving] = useState(false);
    const handleSubmit = async (e: React.FormEvent) => { e.preventDefault(); setIsSaving(true); await onSave(formData); setIsSaving(false); };
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-fade-in">
...
export const LootBoxFormModal = ({ lootBox, onClose, onSave }: { lootBox: Partial<LootBox> | null, onClose: () => void, onSave: (l: any) => void }) => {
    const [formData, setFormData] = useState<Partial<LootBox>>(lootBox || { name: '', price: 0, multiplier: 1, color: '', border_color: '', glow_color: '', icon_type: 'package', potential_rewards: '' });
    const [isSaving, setIsSaving] = useState(false);
    const handleSubmit = async (e: React.FormEvent) => { e.preventDefault(); setIsSaving(true); await onSave(formData); setIsSaving(false); };
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-fade-in">
...
export const SpinWheelItemFormModal = ({ item, onClose, onSave }: { item: Partial<SpinWheelItem> | null, onClose: () => void, onSave: (i: any) => void }) => {
    const [formData, setFormData] = useState<Partial<SpinWheelItem>>(item || { type: 'points', value: 0, text: '', color: '#ffffff', probability: 10, is_active: true });
    const [isSaving, setIsSaving] = useState(false);
    const handleSubmit = async (e: React.FormEvent) => { e.preventDefault(); setIsSaving(true); await onSave(formData); setIsSaving(false); };
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-fade-in">
