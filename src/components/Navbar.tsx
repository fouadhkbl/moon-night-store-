...
const Navbar: React.FC<NavbarProps> = ({ session, onNavigate, cartCount, onSearch, language, setLanguage, onProductSelect }) => {
  ...
  const activities = [
    ...
    {user: "Sarah", action: "just joined the Elite Club!"},
    {user: "Khalid", action: "unlocked a God Pack!"}
  ];
  ...
  return (
    ...
    <nav className="sticky top-0 z-50 bg-[#0b0e14]/90 backdrop-blur-md border-b border-gray-800 h-14 flex items-center shadow-lg">
      <div className="container mx-auto px-4 flex justify-between items-center relative">
        ...
        <div className="hidden xl:flex items-center gap-3 bg-white/5 border border-white/10 px-4 py-1.5 rounded-full overflow-hidden w-64 h-8">
            <Activity className="w-3 h-3 text-green-500 shrink-0 animate-pulse" />
        ...
        <div ref={searchContainerRef} className="hidden lg:flex flex-1 max-w-[280px] flex-col mx-4">
          ...
          {showResults && searchResults.length > 0 && (
              <div className="absolute top-full left-0 w-full bg-[#1e232e] border border-gray-800 rounded-b shadow-2xl overflow-hidden z-50">
                  {searchResults.map(p => (
                      <div key={p.id} onClick={() => { onProductSelect ? onProductSelect(p) : onSearch(p.name); setShowResults(false); }} className="px-2 py-1.5 hover:bg-[#2a303c] cursor-pointer flex items-center gap-2 border-b border-gray-800/50 last:border-0">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button onClick={() => onNavigate('dashboard')} className={`flex items-center gap-2 border p-0.5 pr-2 rounded-full transition shadow-lg relative group ${profile?.vip_level ? 'bg-yellow-900/10 border-yellow-500/50' : 'bg-[#151a23] border-gray-800'}`}>
                {/* Elite Frame FX */}
                {profile?.vip_level ? (
                ...
                <div className={`w-6 h-6 rounded-full overflow-hidden relative z-10 ${profile?.vip_level ? 'border border-yellow-500 shadow-[0_0_10px_rgba(234,179,8,0.4)]' : 'bg-gray-800'}`}>
                ...
                <span className={`text-[9px] font-black max-w-[50px] truncate hidden md:block relative z-10 ${profile?.vip_level ? 'text-yellow-400' : 'text-white'}`}>{profile?.username || 'GUEST'}</span>
          </button>
        </div>
      </div>
    </nav>

    {/* Sider Drawer Menu (No changes here, kept from previous version) */}
                ...
                {!isGuest && profile ? (
                    <div className="space-y-3">
                        <div className="flex items-center gap-3">
                            <div className={`w-10 h-10 rounded-xl border-2 overflow-hidden shadow-lg relative ${profile.vip_level ? 'border-yellow-500' : 'border-blue-500'}`}>
                    ...
                    <div className="py-2">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            <button onClick={() => { onNavigate('dashboard'); setIsMenuOpen(false); }} className="bg-blue-600 text-white py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest flex items-center justify-center gap-1 transition-all active:scale-95"><LogIn className="w-2.5 h-2.5" /> Login</button>
                            <button onClick={() => { onNavigate('dashboard'); setIsMenuOpen(false); }} className="bg-white/10 text-white py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest flex items-center justify-center gap-1 transition-all active:scale-95 border border-white/10"><UserPlus className="w-2.5 h-2.5" /> Join</button>
                        </div>
                    </div>
                )}
            </div>
            <div className="flex-1 overflow-y-auto custom-scrollbar py-2">
                <div className="px-4 py-2">
                    ...
                    <nav className="space-y-1">
                        {menuItems.map(item => (
                            <button key={item.id} onClick={() => { onNavigate(item.id); setIsMenuOpen(false); }} className="w-full flex items-center gap-3 p-2 rounded-lg text-gray-400 hover:text-white hover:bg-white/5 transition-all group border border-transparent hover:border-white/5">
                                <div className={`p-1.5 rounded-md bg-white/5 group-hover:scale-110 transition-transform ${item.color}`}><item.icon className="w-3.5 h-3.5" /></div>
                ...
                <div className="px-4 py-4 mt-2 border-t border-gray-800/50">
                    ...
                    <nav className="space-y-1">
                        <button onClick={() => { onNavigate('dashboard'); setIsMenuOpen(false); }} className="w-full flex items-center gap-3 p-2 rounded-lg text-gray-400 hover:text-white hover:bg-white/5 transition-all group"><LayoutDashboard className="w-3.5 h-3.5 text-blue-500" /><span className="text-[10px] font-bold uppercase tracking-widest">My Dashboard</span></button>
                        {!isGuest && <button onClick={() => { supabase.auth.signOut(); window.location.reload(); }} className="w-full flex items-center gap-3 p-2 rounded-lg text-red-500 hover:bg-red-500/10 transition-all group"><LogOut className="w-3.5 h-3.5" /><span className="text-[10px] font-bold uppercase tracking-widest">Sign Out</span></button>}
                    </nav>
                </div>
            </div>
